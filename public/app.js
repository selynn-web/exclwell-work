...
(function(){
  ...
  var MODULES = {
    ...
    calibrations:{
      ...
      extraActions:function(r){
        ...
        return [{ label:"🔲 "+T3("二维码","QR Code","Kod QR"), onclick:"app.openEquipmentPanel("+jsAttr(r.equipment)+")" }];
      },
      // Flagged by the daily cron job when it auto-creates this record as
      // the "next" calibration draft for a piece of equipment whose
      // previous nextDueDate just arrived (see createDueCalibrationDrafts
      // in api/cron/reminders.js) — a visible nudge that it still needs a
      // real date and result filled in.
      secondaryBadge:function(r){
        if(!r.autoGenerated) return null;
        return { text: T3("自动生成的草稿","Auto-generated draft","Draf dijana automatik"), color:"accent" };
      },
      extraBadge:function(r){
    ...
    repairs:{
      ...
      extraActions:function(r){
        ...
        if(r.equipment) actions.push({ label:"🔲 "+T3("二维码","QR Code","Kod QR"), onclick:"app.openEquipmentPanel("+jsAttr(r.equipment)+")" });
        return actions;
      },
      metaLines:function(r){
        return [
  ...
  // Safely embeds an arbitrary string (equipment names can contain quotes,
  // apostrophes, etc.) into an onclick="..." HTML attribute. JSON.stringify
  // produces a JS string literal (with its own \" and \\ escaping), and esc()
  // then HTML-escapes that literal so the browser's HTML parser hands the
  // *exact* original JS source back to the inline-script parser. Existing
  // onclick handlers elsewhere in this file that instead do \''+esc(x)+'\'
  // predate this helper and break on names containing an apostrophe — new
  // free-text embeddings (like equipment names below) should use this
  // instead rather than that older pattern.
  function jsAttr(v){ return esc(JSON.stringify(v)); }
  ...
  function renderModal(){
    ...
    if(!UI.modal){
      if(UI.equipmentPanel) return renderEquipmentPanelModal();
      return "";
    }
    ...
    // A lightweight discussion thread per record — separate from the audit
    // trail above (see the matching server-side comment on MAX_COMMENTS in
    // api/state.js). Only shown for an existing, already-saved record: a
    // brand-new unsaved record has no id yet for a comment to attach to.
    var commentsHtml = "";
    if(isEdit){
      var wdc = writeDisabled();
      var comments = Array.isArray(baseRecord.comments) ? baseRecord.comments : [];
      var commentRows = comments.map(function(c){
        var when = c.at ? c.at.slice(0,16).replace("T"," ") : "";
        return '<li class="comment-row">'
          + '<div class="comment-meta"><span class="comment-author">'+esc(c.by||"")+'</span><span class="comment-time">'+esc(when)+'</span>'
          + '<button type="button" class="btn btn-ghost btn-sm comment-del" onclick="app.deleteComment(\''+mod.key+'\',\''+baseRecord.id+'\',\''+c.id+'\')" '+wdc+'>'+esc(T("删除"))+'</button></div>'
          + '<div class="comment-text">'+esc(c.text||"")+'</div></li>';
      }).join("");
      commentsHtml = '<div class="comments-section">'
        + '<h4 class="comments-title">'+esc(T3("留言","Comments","Ulasan"))+(comments.length?(' ('+comments.length+')'):'')+'</h4>'
        + (commentRows ? ('<ul class="comment-list">'+commentRows+'</ul>') : ('<p class="comments-empty">'+esc(T3("暂无留言，可在此和同事讨论这条记录。","No comments yet — discuss this record with your team here.","Belum ada ulasan — bincangkan rekod ini dengan rakan sekerja di sini."))+'</p>'))
        + '<form class="comment-form" onsubmit="app.submitComment(event,\''+mod.key+'\',\''+baseRecord.id+'\')">'
        + '<textarea name="commentText" class="input comment-input" rows="2" placeholder="'+esc(T3("写留言…","Write a comment…","Tulis ulasan…"))+'" '+wdc+'></textarea>'
        + '<button type="submit" class="btn btn-ghost btn-sm" '+wdc+'>'+esc(T("发送"))+'</button>'
        + '</form></div>';
    }
  ...
  /* ---------- Overview dashboard charts (hand-rolled inline SVG, no
     external chart library) ----------
     Every fill uses the same CSS custom properties (var(--accent) etc.)
     the rest of the app already uses for status pills, so these charts
     automatically track the current light/dark theme with no extra work.
     Kept deliberately simple (bars, not anything interactive) — this is a
     small team's daily-glance dashboard, not an analytics product. */

  var MONTH_LABELS_ZH = ["1月","2月","3月","4月","5月","6月","7月","8月","9月","10月","11月","12月"];
  var MONTH_LABELS_EN = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];

  function svgModuleBarChart(rows){
    // rows: [{label, value}], already sorted/filtered by the caller.
    var max = Math.max(1, Math.max.apply(null, rows.map(function(r){ return r.value; })));
    var barH = 22, rowGap = 12, labelW = 104, chartW = 260, valueGap = 8;
    var height = rows.length * (barH+rowGap) - rowGap;
    var bars = rows.map(function(r, i){
      var y = i*(barH+rowGap);
      var w = r.value > 0 ? Math.max(3, Math.round((r.value/max) * chartW)) : 0;
      return '<text x="'+labelW+'" y="'+(y+barH/2)+'" text-anchor="end" dy="0.35em" class="chart-label">'+esc(r.label)+'</text>'
        + '<rect x="'+(labelW+valueGap)+'" y="'+y+'" width="'+w+'" height="'+barH+'" rx="4" style="fill:var(--accent)" opacity="0.82"></rect>'
        + '<text x="'+(labelW+valueGap+w+7)+'" y="'+(y+barH/2)+'" dy="0.35em" class="chart-value">'+r.value+'</text>';
    }).join("");
    var totalW = labelW+valueGap+chartW+34;
    return '<svg class="chart-svg" viewBox="0 0 '+totalW+' '+height+'" width="100%" height="'+height+'" role="img" aria-label="'+esc(T3("各模块记录数量","Records per module","Rekod setiap modul"))+'">'+bars+'</svg>';
  }

  function svgStatusBar(counts, colorMap){
    // counts: [{status, value}] — renders one horizontal bar split into
    // colored segments by status, proportional to share of total.
    var total = counts.reduce(function(s,c){ return s+c.value; }, 0);
    var barW = 320, barH = 26;
    if(!total){
      return '<svg class="chart-svg" viewBox="0 0 '+barW+' '+barH+'" width="100%" height="'+barH+'"><rect x="0" y="0" width="'+barW+'" height="'+barH+'" rx="6" style="fill:var(--surface-2)"></rect></svg>';
    }
    var x = 0;
    var segs = counts.filter(function(c){ return c.value>0; }).map(function(c){
      var w = Math.round((c.value/total)*barW);
      var seg = '<rect x="'+x+'" y="0" width="'+w+'" height="'+barH+'" style="fill:var(--'+(colorMap[c.status]||"neutral")+')"></rect>';
      x += w;
      return seg;
    }).join("");
    return '<svg class="chart-svg" viewBox="0 0 '+barW+' '+barH+'" width="100%" height="'+barH+'" role="img" aria-label="'+esc(T3("追踪记录状态分布","Tracker status breakdown","Taburan status penjejakan"))+'"><rect x="0" y="0" width="'+barW+'" height="'+barH+'" rx="6" style="fill:var(--surface-2)"></rect>'+segs+'<rect x="0.5" y="0.5" width="'+(barW-1)+'" height="'+(barH-1)+'" rx="6" fill="none" stroke="var(--line)"></rect></svg>';
  }

  function svgMonthlyTrend(months){
    // months: [{label, value}], oldest first.
    var max = Math.max(1, Math.max.apply(null, months.map(function(m){ return m.value; })));
    var barW = 34, gap = 16, chartH = 84;
    var totalW = months.length*(barW+gap)-gap;
    var totalH = chartH+38;
    var bars = months.map(function(m, i){
      var x = i*(barW+gap);
      var h = m.value > 0 ? Math.max(3, Math.round((m.value/max)*chartH)) : 0;
      var y = chartH - h;
      return '<rect x="'+x+'" y="'+y+'" width="'+barW+'" height="'+h+'" rx="4" style="fill:var(--accent)" opacity="0.82"></rect>'
        + '<text x="'+(x+barW/2)+'" y="'+(chartH+16)+'" text-anchor="middle" class="chart-value">'+m.value+'</text>'
        + '<text x="'+(x+barW/2)+'" y="'+(chartH+31)+'" text-anchor="middle" class="chart-label">'+esc(m.label)+'</text>';
    }).join("");
    return '<svg class="chart-svg" viewBox="0 0 '+totalW+' '+totalH+'" width="100%" height="'+totalH+'" role="img" aria-label="'+esc(T3("产品损毁记录近 6 个月趋势","Damage records, last 6 months","Rekod kerosakan produk, 6 bulan lepas"))+'">'+bars+'</svg>';
  }

  // Buckets `records` (using dateField, falling back to createdAt) into the
  // last 6 calendar months including the current one. Records with no
  // parseable date in either field are simply not counted — same
  // "don't count what we can't place" approach as the Gantt chart's
  // no-due-date note.
  function lastSixMonthsCounts(records, dateField){
    var now = new Date();
    var buckets = [];
    for(var i=5; i>=0; i--){
    ...
    records.forEach(function(r){
      var raw = r[dateField] || r.createdAt;
      if(!raw) return;
    ...
    var labels = LANG === "zh" ? MONTH_LABELS_ZH : MONTH_LABELS_EN;
    return buckets.map(function(b){ return { label: labels[b.m], value: b.value }; });
  }
  ...
  function renderDashboardCharts(){
    ...
    var moduleTotals = [
      {key:"meetings", label:T("会议记录"), value:(STATE.meetings||[]).length},
      ...
      {key:"vehicles", label:T("汽车管理"), value:(STATE.vehicles||[]).length},
      {key:"damages", label:T("产品损毁记录"), value:(STATE.damages||[]).length},
      {key:"trackers", label:T("追踪记录"), value:(STATE.trackers||[]).length}
    ].filter(function(r){ return isModuleAllowed(r.key); });
    if(moduleTotals.some(function(r){ return r.value > 0; })){
      sections.push('<div class="chart-block"><h4 class="chart-title">'+esc(T3("各模块记录数量","Records per module","Rekod setiap modul"))+'</h4>'+svgModuleBarChart(moduleTotals)+'</div>');
    }

    if(isModuleAllowed("trackers") && (STATE.trackers||[]).length){
      var tStatuses = ["待处理","进行中","已完成","已延误"];
      var tCounts = tStatuses.map(function(s){ return { status:s, value:(STATE.trackers||[]).filter(function(r){ return r.status===s; }).length }; });
      var legend = tCounts.filter(function(c){ return c.value>0; }).map(function(c){
        return '<div class="legend-item"><span class="legend-dot" style="background:var(--'+(MODULES.trackers.statusColors[c.status]||"neutral")+')"></span>'+esc(T(c.status))+' '+c.value+'</div>';
      }).join("");
      sections.push('<div class="chart-block"><h4 class="chart-title">'+esc(T3("追踪记录状态分布","Tracker status breakdown","Taburan status penjejakan"))+'</h4>'+svgStatusBar(tCounts, MODULES.trackers.statusColors)+'<div class="chart-legend">'+legend+'</div></div>');
    }

    if(isModuleAllowed("damages") && (STATE.damages||[]).length){
      var months = lastSixMonthsCounts(STATE.damages||[], "date");
      sections.push('<div class="chart-block"><h4 class="chart-title">'+esc(T3("产品损毁记录近 6 个月趋势","Damage records, last 6 months","Rekod kerosakan produk, 6 bulan lepas"))+'</h4>'+svgMonthlyTrend(months)+'</div>');
    }

    if(!sections.length) return "";
    return '<div class="panel dashboard-charts"><h3 class="panel-title">'+esc(T3("数据概览","Data overview","Gambaran keseluruhan data"))+'</h3><div class="chart-grid">'+sections.join("")+'</div></div>';
  }
  ...
  function setView(key){
    ...
    UI.view = key; UI.modal = null; UI.confirmDelete = null; UI.equipmentPanel = null;
    render();
  ...
  /* ---------- equipment QR panel ---------- */
  // Trim + lowercase so "Mixer 1", " mixer 1 ", "MIXER 1" etc. are treated
  // as the same piece of equipment when matching repairs/calibrations
  // records — free-text entry across different forms/people will never be
  // byte-identical otherwise.
  function normalizeEquip(s){ return String(s||"").trim().toLowerCase(); }

  function openEquipmentPanel(name){
    if(!name) return;
    UI.equipmentPanel = { equipment: name };
    render();
  }
  function closeEquipmentPanel(){ UI.equipmentPanel = null; render(); }

  function equipmentQrUrl(name){
    return location.origin + location.pathname + "?equip=" + encodeURIComponent(name);
  }

  function equipmentQrSvg(name){
    try{
      if(typeof qrcode !== "function") return "";
      var qr = qrcode(0, "M");
      qr.addData(equipmentQrUrl(name));
      qr.make();
  ...
  function equipmentRecordRow(moduleKey, r){
    var wd = writeDisabled();
    var line2 = moduleKey === "repairs" ? (r.issue||"") : (r.result || (r.type?T(r.type):""));
    return '<div class="equip-record-row">'
      + '<div class="equip-record-top"><span>'+esc(T(r.id))+' · '+esc(T(r.status||""))+'</span>'
      + '<button type="button" class="btn btn-ghost btn-sm" onclick="app.closeEquipmentPanel();app.openModal(\''+moduleKey+'\',\''+r.id+'\')" '+wd+'>'+esc(T("编辑"))+'</button></div>'
      + '<div class="equip-record-meta">'+esc([r.date||"", line2].filter(Boolean).join(" · "))+'</div>'
      + '</div>';
  }

  // Deliberately uses jsAttr() (not the raw \''+esc(x)+'\' pattern used
  // elsewhere in this file) because equipment names are free text a person
  // typed into a form field — they can contain apostrophes or double quotes,
  // and that older pattern breaks (or, worse, injects) on those. See jsAttr's
  // own comment above esc() for why this is safe.
  function renderEquipmentPanelModal(){
    var name = UI.equipmentPanel.equipment;
    var wd = writeDisabled();
    var svg = equipmentQrSvg(name);
    var url = equipmentQrUrl(name);

    var moduleSections = [
      {key:"repairs", label:"设备维修记录", newLabel:"新建维修记录"},
      {key:"calibrations", label:"设备校准记录", newLabel:"新建校准记录"}
    ];
    var sectionsHtml = moduleSections.map(function(cfg){
      if(!isModuleAllowed(cfg.key)) return "";
      var rows = (STATE[cfg.key]||[]).filter(function(r){ return normalizeEquip(r.equipment) === normalizeEquip(name); });
      rows.sort(function(a,b){ return (b.date||"").localeCompare(a.date||""); });
      var listHtml = rows.length
        ? rows.slice(0,8).map(function(r){ return equipmentRecordRow(cfg.key, r); }).join("")
        : '<div class="empty-state-sm">'+esc(T("暂无相关记录"))+'</div>';
      return '<div class="equip-section qr-noprint">'
        + '<div class="equip-section-title"><span>'+esc(T(cfg.label))+' ('+rows.length+')</span>'
        + '<button type="button" class="btn btn-ghost btn-sm" onclick="app.closeEquipmentPanel();app.openModal(\''+cfg.key+'\',null,'+jsAttr({equipment:name})+')" '+wd+'>+ '+esc(T(cfg.newLabel))+'</button></div>'
        + listHtml
        + '</div>';
    }).join("");

    var qrHtml = svg
      ? '<div class="equip-qr-svg">'+svg+'</div>'
      : '<div class="equip-qr-svg empty-state-sm">'+esc(T("二维码生成失败"))+'</div>';

    return '<div class="overlay" onclick="app.closeEquipmentPanel()"><div class="modal modal-lg qr-print-area" onclick="event.stopPropagation()">'
      + '<h3 class="modal-title qr-noprint">'+esc(T("设备二维码"))+'</h3>'
      + '<div class="equip-qr-wrap">'
      + qrHtml
      + '<div class="equip-qr-meta"><h4>'+esc(name)+'</h4>'
      + '<p class="qr-noprint">'+esc(T("扫描此码可直接打开该设备的维修/校准记录。建议打印后贴在设备上。"))+'</p>'
      + '<p class="qr-noprint">'+esc(url)+'</p>'
      + '<div class="equip-qr-actions qr-noprint">'
      + '<button type="button" class="btn btn-primary btn-sm" onclick="window.print()">🖨 '+esc(T("打印"))+'</button>'
      + '<button type="button" class="btn btn-ghost btn-sm" onclick="app.closeEquipmentPanel()">'+esc(T("关闭"))+'</button>'
      + '</div></div></div>'
      + sectionsHtml
      + '</div></div>';
  }
  ...
  function submitComment(e, moduleKey, recordId){
    e.preventDefault();
    var form = e.target;
    var input = form.elements["commentText"];
    var text = input ? String(input.value).trim() : "";
    if(!text) return;
    var before = deepClone(STATE);
    var arr = STATE[moduleKey];
    var idx = arr.findIndex(function(r){ return r.id === recordId; });
    if(idx > -1){
      var optimistic = { id:"（发送中…）", at:new Date().toISOString(), by:(CURRENT_USER&&CURRENT_USER.name)||"", text:text };
      var comments = (Array.isArray(arr[idx].comments) ? arr[idx].comments.slice() : []).concat([optimistic]);
      arr[idx] = Object.assign({}, arr[idx], {comments:comments});
    }
    // The comment textarea isn't bound to STATE — render() below rebuilds
    // it fresh (empty) from this same template, which is what clears it
    // after sending, same as the record form's own inputs elsewhere.
    render();
    saveOp({op:"comment", module:moduleKey, id:recordId, text:text}, before);
  }

  function deleteComment(moduleKey, recordId, commentId){
    var before = deepClone(STATE);
    var arr = STATE[moduleKey];
    var idx = arr.findIndex(function(r){ return r.id === recordId; });
    if(idx > -1){
      var comments = (Array.isArray(arr[idx].comments) ? arr[idx].comments : []).filter(function(c){ return c.id !== commentId; });
      arr[idx] = Object.assign({}, arr[idx], {comments:comments});
    }
    render();
    saveOp({op:"deleteComment", module:moduleKey, id:recordId, commentId:commentId}, before);
  }
  ...
  function boot(){
    ...
    fetchState().then(function(json){
      ...
      mode = "writer";
      render();
      // A scanned equipment QR code lands here as ?equip=<name> — open that
      // equipment's panel once, then strip the query string so a page
      // refresh (or re-sharing the URL) doesn't reopen it unexpectedly.
      // Safe to do after login too: submitLogin()/submitJoin() call
      // hideLogin()+boot() in place with no navigation, so location.search
      // survives an unauthenticated scan all the way through to here.
      try{
        var equipParam = new URLSearchParams(location.search).get("equip");
        if(equipParam){
          history.replaceState(null, "", location.pathname);
          openEquipmentPanel(equipParam);
        }
      }catch(err){}
